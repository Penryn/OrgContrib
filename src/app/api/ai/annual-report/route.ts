...
type ApiResponse =
  ...
  | {
      error: "unauthorized" | "invalid_year" | "cache_not_ready" | "internal_error";
      message?: string;
      hints?: string[];
    };
...
export async function POST() {
  try {
    ...
    console.error("POST /api/ai/annual-report failed", { message });
    ...
    return NextResponse.json(
      ...
      { status: includesFetchFailedMessage(err) ? 503 : 500 },
    );
  }
}
